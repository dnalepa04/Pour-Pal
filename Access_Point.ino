...
// Pour Pal Cocktail Dispenser
// ESP32 Access Point with Flash Storage

//Import libraries
#include <WiFi.h> // Create WI-FI Functionality
#include <WebServer.h> // Create Web Server
#include <ArduinoJson.h> // JSON Formating Data
#include <LiquidCrystal_I2C.h> // I2C Communication
...
// Create Structure for User Stored Cocktails
struct Cocktail {
  String name;
  int volumes[6];
  String pumpNames[6];
...
// Indexing Structure
// 2*idx first pint, 2*idx +1 second pin
// indx 0 --> Pump1 --> IN Pins 3/4
// indx 1 --> Pump2 --> IN Pins 5/6
// indx 2 --> Pump3 --> IN Pins 7/8
// indx 3 --> Pump4 --> IN Pins 9/10
// indx 4 --> Pump5 --> IN Pins A0/A1
// indx 5 --> Pump6 --> IN Pins A2/A3
...
// Run Pumps to Dispense
void hardwareStartPump(int idx) {
  digitalWrite(inPins[2*idx], LOW);
  digitalWrite(inPins[2*idx + 1], HIGH);
}

// Run Pumps to Retract
void hardwareReversePump(int idx) {
  digitalWrite(inPins[2*idx], HIGH);
  digitalWrite(inPins[2*idx + 1], LOW);
}

// Runs Pump for Defined Amount of Fluid
void startPump(int idx, int volume_mL) {
  if(idx<0||idx>=6||volume_mL<=0) return; // Checks for Correct Indexing
  pumpVolumes[idx] = volume_mL; // Store Requested Volume for said Pump
  pumpRunning[idx] = true; // Set state of Pump running to true
  unsigned long duration = (unsigned long)((volume_mL / pumpFlowRate[idx])*1000.0); // Calculate Time to Run Pump
  pumpEndTime[idx] = millis() + duration; // Future Time to Stop Pump
  hardwareStartPump(idx); // Run Pump
}

 // Stop Pump and Reset State
void stopPump(int idx) {
  if(idx<0||idx>=6) return; // Checks for Correct Indexing
  pumpRunning[idx] = false; // Set state of Pump running to false
  pumpVolumes[idx] = 0; // Reseet Requested Volume
  pumpEndTime[idx] = 0; // Reset Future Time
  hardwareStopPump(idx); // Stop Pump
}

// Save User Cocktails into Flash Memory using JSON Strings
...
// Save User Cocktails From Flash Memory using JSON Strings
...
// HTML Page for Cocktail Dispenser UI
void handleRoot() {
...
// Return Status of Pour Pal as JSON String
void handleStatus() {
...
// Return Stored Cotails as JSON Array to Populate UI
...
// Dispense Manual Volumes from User
void handleDispense(){
  for(int i=0;i<6;i++){
...
void setup(){
  ...
  //Initialize LCD and Perform Startup
  lcd.init();
  lcd.backlight();
  lcd.clear();
  lcd.setCursor(0,0);
  ...
  //Load User Stored Cocktails
  loadCocktailsFromFlash();

  // Start Web Endpoints and Server
...
void loop(){
  // Handle Request from Web UI
  server.handleClient();

  // Store Current Time
  unsigned long now=millis();

  // Check if Pumps Need to Stop
  for(int i=0;i<6;i++){
